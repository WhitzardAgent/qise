...
Qise 可以帮助你在使用 Codex、OpenClaw 以及 OpenAI-compatible 自定义 Agent 时，增加一层本地安全防护：扫描集成、把模型流量接入本地 Guard Proxy、拦截高风险动作，并留下可解释的安全事件记录。
...
| Proxy 防护 | 面向 Codex、OpenClaw 和自定义 Agent 的本地 OpenAI-compatible proxy | MVP 可用 |
...
| Claude Code | 实验支持；原生 Anthropic `/v1/messages` proxy 尚未完成 | Experimental |
...
src/qise/proxy/        OpenAI-compatible 本地 proxy 和 streaming 支持
...
- 只有在保护真实 Codex/OpenClaw/custom Agent 时，才需要真实 Agent 安装。
...
- 模型服务商 API key 仍然存在于 Agent 使用的环境里，例如 `OPENAI_API_KEY`。
...
| `qise proxy start` | 手动启动本地 OpenAI-compatible proxy。 |
...
如果你正在开发 Agent，并希望在工具、输入、输出或框架 hook 周围做进程内检查，使用 adapter。若你希望零代码保护一个现有 OpenAI-compatible Agent，则优先使用 proxy 模式。
...
- 在包发布完成前，源码安装是主要支持路径。
- Proxy 模式当前主要面向 OpenAI-compatible chat/completions 风格流量。
- Claude Code 仍是实验支持，因为原生 Anthropic `/v1/messages` 拦截尚未完成。
...
- [Claude Code 状态](./docs/claude-code.md)
